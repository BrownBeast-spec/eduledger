...
class VerifierPortal {
    constructor(verifierId, verifierName) {
    ...
    async verifyCertificate() {
        ...
        try {
            ...
            const consentValid = await this.consentContract.evaluateTransaction('VerifyConsent', consentId, this.verifierId);
            if (consentValid.toString() !== 'true') {
                console.log('✗ Invalid or expired consent');
                return;
            }
            // Get consent details to see what data can be accessed
            const consentResult = await this.consentContract.evaluateTransaction('ReadConsent', consentId);
            ...
            consent.dataShared.forEach((field) => {
    ...
    async viewMyConsents() {
        ...
        try {
            const result = await this.consentContract.evaluateTransaction('QueryConsentsByVerifier', this.verifierId);
    ...
    async viewCertificateHistory() {
        ...
        try {
            ...
            const result = await this.certContract.evaluateTransaction('GetCertificateHistory', certId);
